function [f_loadS_OFF ,data_unload_final] = ContactPoint_sort(smoothed_load, smoothed_unloadS)
...
% smoothed_unloadS=smoothed_unloadS(1:length(smoothed_load(:,1)),:);
...
sub1_unload(:,1) = smoothed_unloadS(:,2) - ytemp(:,1); % UNLOAD
...
% plot(smoothed_load(:,1),smoothed_load(:,2),'-r','LineWidth',1.5)
% plot(smoothed_unloadS(:,1),smoothed_unloadS(:,2),'-g','LineWidth',1.5)
...
moving_average(:,1)=sort(smooth(sub1(:,1),'moving'));
...
sub2(:,1) = sub1(:,1) - ytemp2(:,1); % LOAD
sub1_unload(:,1) = smoothed_unloadS(:,2) - ytemp2(:,1); % UNLOAD
data_unload2(:,2) = data_unload(:,2) - ytemp2(:,1);
data_unload2(:,1) = data_unload(:,1);
moving_average2(:,1)=moving_average(:,1) - ytemp2(:,1);
...
% grid on
% plot(smoothed_load(:,1),sub1(:,1),'-r','LineWidth',1.5)
% plot(smoothed_load(:,1),moving_average2(:,1),'-o','LineWidth',1.5)
...
dy = diff(moving_average2(:,1)); % this is the moving average of the difference in y.
% dx = diff(smoothed_load(:,1));
% deriv1 = dy./dx; 
smoothedDiff=smooth(dy(:,1),'moving');
MeanDiff=mean(smoothedDiff(1:index,1));
...
% setfigureoptions(10,10)
% plot(smoothed_load(:,1),moving_average2(:,1),'-g','linewidth',2)
% plot(smoothed_load(1:end-1,1),dy(:,1),'ob')
% plot(smoothed_load(1:end-1,1),smoothedDiff(:,1),'-or')
% plot([3.45 3.654],[mean(dy(1:index,1)) mean(dy(1:index,1))],'b')
% plot([3.45 3.654],[mean(dy(1:index,1))+SD6times mean(dy(1:index,1))+SD6times],'--k')
%  plot(smoothed_load(1:end-1,1),smooth(dy(:,1),'moving'),'r')
% plot([3.45 3.654],[mean(dy(1:index,1))-SD6times mean(dy(1:index,1))-SD6times],'--k')
% plot([smoothed_load(position,1) smoothed_load(position,1)],[min(moving_average2(:,1)) 0.5*max(moving_average2(:,1))])
% plot(smoothed_load(1:end-1,1),dy(:,1),'LineWidth',2)
...
% plot(fdata_loadS{j,1}(1:length(deriv1),1),flippedDeriv,'og','LineWidth',1.5)
...
%  legend('-','-','1st derivative','smoothed derivative','original y-offset data')
 ...
 yoffset = moving_average2(position,1);
...
f_loadS_OFF(:,2) = sub2(:,1) - yoffset;
data_unload_final(:,1) = data_unload2(:,1)-xoffset;
data_unload_final(:,2) = data_unload2(:,2) - yoffset;
% data_unload(:,2) = data_unload(:,2)-yoffset;
% setfigureoptions(10,10)
% plot(f_loadS_OFF(:,1),f_loadS_OFF(:,2),'-b')
% plot(data_unload_final(:,1),data_unload_final(:,2),'-m')
...
clear moving_average moving_average2 dy dx deriv1 sub1 sub2 sub3 xmin xmax temp x y p1...
