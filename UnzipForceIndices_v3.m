...
function [flistforce,flistmap,Status] = UnzipForceIndices_v3(datadir,System,indices,fnamemap,tempdir)
N=nargin;
%direct=cd;
...
% is a message indicating unziping was successful.
    ...
    if count==length(flistmap)
        Status='OK';
        disp('Unzip of .force curves OK');
    else
        Status='FAIL';
        disp('There is something wrong with the amount of .force files');
    end
end    
...
if N==5
    ...
    elseif strcmp(System,'L')==1;
                ...
                outfolder = tempdir;
                 match = ['index/', num2str(indices(i)), '*']
                cmd = ['unzip -o ',zipfile, ' ''',match, ''' -d ' tempdir]
                system(cmd);
       elseif strcmp(System,'M')==1;
                ...
                outfolder = tempdir;
                 match = ['index/', num2str(indices(i)), '*']
              cmd = ['unzip -o ',zipfile, ' ''',match, ''' -d ' tempdir, ' -o']
                system(cmd);
