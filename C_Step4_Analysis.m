...
for j = 1 : length(indices);  
    ...
    headerDir = 'shared-data\header.properties';
    headerDir = fullfile(tempdir{fib,1},headerDir);

    
    fdata_load{j,1} = writedata(headerDir,segHeaderDir,heightdir,vDefldir);
...
% contact point.
    scaling=1e6;
    [approach{j,1} ,retract{j,1}] = ContactPoint(...
...
clear choise ans zipContents fdata_load data_unload fdata_loadS fdata_unload ...
...
save(sprintf('%s%s%s',resultsFolder,'\','selected_data_for_analysis.mat'))
