...
% Save the current figure in the FIBRIL_0X subfolder of the RESULTS folder.
    ...
    figname=sprintf('%s%sEGMaxFl_0%d_line%d'...
                      ,resultsFolder,'\',fib,lineprofileIndex);
    f = fullfile(figname);
    saveFigures(gcf,f); % save these figures for post process analysis.
    close
...
save(sprintf('%s%ssteps1to3_analysis.mat',datadir,'\')) 
%% step 4 analysis
...
zipContents = listzipcontents(sprintf('%s%s%s',datadir,'/',fnamemap));
