function plot_linear_fit(approach_X,approach_Y,retract_X,...
                            retract_Y,percent,figure_size,name,n,folder)
...
%% plots data
plot(pos_approach(:,1),pos_approach(:,2),pos_retract(:,1),pos_retract(:,2))
plot(x,y,'-','Color',[0.8 0.2 0.8])
plot(pos_retract(1:row,1),pos_retract(1:row,2),'Color',[0.2 0.6 0.2])
%% adds labels
xlabel('Height (\mum)','FontSize',8);
ylabel('Deflection (\mum)','FontSize',8);
